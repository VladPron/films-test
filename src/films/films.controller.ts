...
export class FilmsController {
    ...
    async create(@Request() req, @Body() createFilmDTO: CreateFilmDTO): Promise<Film> {
        try {
    ...
    async update(@Param('id') id: number, @Body() updateFilmDTO: UpdateFilmDTO): Promise<Film> {
        return await this.filmsService.update(+id, updateFilmDTO);
    }
 
    @Post(':id/vote')
    async createVote(@Request() req, @Param('id') id: number, @Body() voteDTO: VoteDTO): Promise<any> {
        return await this.filmsService.createVote(req.user.userId, +id, +voteDTO.value);
    }
}
