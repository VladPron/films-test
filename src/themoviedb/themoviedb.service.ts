...
export class MovieDBService {
    ...
    async fetch(userId: number, id: number): Promise<List> {
        const { data } = await this.mdb.list.getDetails({ pathParameters: { list_id: id } });

        let list = await this.listService.getByName(data.name);
        if (!list) {
            list = await this.listService.create(userId, { name: data.name, description: data.description });
        }

        await data.items.map(async (movie) => {
            let film = await this.filmService.getByName(movie.title);
            
            if (!film) {
                const { data } = await this.mdb.movie.getDetails({ pathParameters: { movie_id: movie.id } })        
                ...
                const createDTO: CreateFilmDTO = {
                    ...
                    listId: list.id,
                    country: data.production_companies[0].name,
